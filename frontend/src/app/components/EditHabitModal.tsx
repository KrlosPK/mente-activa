...
interface EditHabitModalProps {
  habit: Habit;
  onClose: () => void;
  onUpdate: (habitId: string, name: string, category: string, frequency: string, reminderEnabled: boolean, reminderTime: string) => void;
}

export function EditHabitModal({ habit, onClose, onUpdate }: EditHabitModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      onUpdate(habit.id, name, category, frequency, reminderEnabled, reminderTime);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-6 z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <h3 className="text-xl font-semibold text-gray-900">Editar Hábito</h3>
          <button
            onClick={onClose}
